    ...
    def transform(self, X, y=None):
        if self.param_type == "euler":
            ...
            raise UnsupportedParamError(
                "Unsupported param: %s. Valid param types are: euler, quat, expmap, position"
                % self.param_type
            )
        ...
        for track in X:
            ...
            for joint in track.traverse():
                ...
                if rc.shape[1] < 3:
                    ...
                    rotation_order = (
                        rc.columns[0][rc.columns[0].find("rotation") - 1]
                        + rc.columns[1][rc.columns[1].find("rotation") - 1]
                        + rc.columns[2][rc.columns[2].find("rotation") - 1]
                    )  # rotation_order is string : 'XYZ' or'ZYX' or ...
                    ####################################################################################################
                ...
                rotmats = np.asarray(
                    [
                        Rotation(
                            [f[0], f[1], f[2]], "euler", rotation_order, from_deg=True
                        ).rotmat
                        for f in euler_values
                    ]
                )
                ########################################################################################################
                tree_data[joint] = [
    ...
    def _expmap_to_euler(self, X):
        ...
        for track in X:
            ...
            for joint in joints:
                ...
                euler_rots = [
                    Rotation(f, "expmap").to_euler(True)[0] for f in expmap
                ]  # Convert the eulers to exp maps

                # Create the corresponding columns in the new DataFrame

                euler_df["%s_Xrotation" % joint] = pd.Series(
                    data=[e[0] for e in euler_rots], index=euler_df.index
                )
                euler_df["%s_Yrotation" % joint] = pd.Series(
                    data=[e[1] for e in euler_rots], index=euler_df.index
                )
                euler_df["%s_Zrotation" % joint] = pd.Series(
    ...
    def transform(self, X, y=None):
        ...
        for joint_name in selected_joints:
            selected_channels.extend(
                [o for o in X[0].values.columns if joint_name in o]
            )
        ...
        for track in X:
            ...
            # end of abdolute_translation_deltas
                ...
                root_pos_x_diff = pd.Series(
                    data=track.values[xp_col].diff(), index=new_df.index
                )
                root_pos_z_diff = pd.Series(
                    data=track.values[zp_col].diff(), index=new_df.index
                )

                root_rot_y_diff = pd.Series(
                    data=track.values[yr_col].diff(), index=new_df.index
                )
                root_rot_x_diff = pd.Series(
                    data=track.values[xr_col].diff(), index=new_df.index
                )
                root_rot_z_diff = pd.Series(
                    data=track.values[zr_col].diff(), index=new_df.index
                )
                ...
                new_df.drop(
                    [xr_col, yr_col, zr_col, xp_col, zp_col], axis=1, inplace=True
                )
    ...
    def inverse_transform(self, X, copy=None, start_pos=None):
        ...
        for track in X:
            ...
            # end of abdolute_translation_deltas
                ...
                new_df.drop(
                    [dxr_col, dyr_col, dzr_col, dxp_col, dzp_col], axis=1, inplace=True
                )
